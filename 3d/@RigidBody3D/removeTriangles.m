function removeTriangles( obj, tris, updateMesh, settings )
    ...
    setDiffIds(obj.VertexIDs) = true;
    setDiffIds(obj.Mesh.t(tris, :)) = false;
    vertices = triangleIdRange(setDiffIds);
%     vertices = setdiff(obj.VertexIDs, [obj.Mesh.t(tris, :)]);

    if numel(vertices) == numel(obj.VertexIDs)
        return;
    end
