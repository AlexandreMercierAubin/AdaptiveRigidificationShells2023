function updateBody(body, updateMesh)
    ...
    if size(body.Mesh.t,2) == 4
        body.TetInds = find(accumarray([body.Mesh.TetsPerParticle{body.VertexIDs}]', 1) == 4)';
    else
        body.TetInds = find(accumarray([body.Mesh.TetsPerParticle{body.VertexIDs}]', 1) == 3)';
    end
    ...
    body.DOFs = reshape([body.VertexIDs * 3 - 2; body.VertexIDs * 3 - 1; body.VertexIDs * 3], 1, []);
    body.isPinned = any(body.Mesh.pinned(body.VertexIDs));
    
    masses = body.Mesh.mass(body.VertexIDs * 3);
    
    body.Mass = sum(masses);
    
    bpx = body.Mesh.p( body.VertexIDs*3-2 );
    bpy = body.Mesh.p( body.VertexIDs*3-1 );
    bpz = body.Mesh.p( body.VertexIDs*3 );
    bvx = body.Mesh.v( body.VertexIDs*3-2 );
    bvy = body.Mesh.v( body.VertexIDs*3-1 );
    bvz = body.Mesh.v( body.VertexIDs*3 );
    ...
    % based on this...
    body.Mesh.VertexDisp(body.VertexIDs, 1) = bpx - body.Position(1);
    body.Mesh.VertexDisp(body.VertexIDs, 2) = bpy - body.Position(2);
    body.Mesh.VertexDisp(body.VertexIDs, 3) = bpz - body.Position(3);
    
    bpcom = body.Mesh.VertexDisp( body.VertexIDs, : ); % body particle positions in CoM frame
    
    body.Inertia = zeros(3,3);
    for i = 1: numel(body.VertexIDs)
        rhat = crossProductMatrix( bpcom(i,:) );
        body.Inertia = body.Inertia - masses(i) * rhat^2;
    end
    
    bvcom = zeros( numel(body.VertexIDs), 3 );     % body particle velocities in CoM frame
