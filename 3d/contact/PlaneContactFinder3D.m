classdef PlaneContactFinder3D < ContactFinder
    ...
    methods
        ...
        function [J, phi, cInfo] = findContacts( obj, meshes, time, p )
            ...
            hitThickness = meshes.vertexContactThickness;
%             hitThickness = zeros(size(meshes.vertexContactThickness));
            % indices of the ones under the plane
            in = distance <= hitThickness;
            if ( all(in == false) )
                J = zeros(0,numel(ps));
                phi = [];
            ...
            phi = distance(in) - hitThickness(in);
