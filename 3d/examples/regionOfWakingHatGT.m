...
be.useQuicksolveContactFilter = 0;
be.Gravity = -9.8;
...
cache.F = mexComputeFtri3D(mesh3Da.getPositionFormatted,mesh3Da.t,mesh3Da.triNormals,mesh3Da.dphidx(:));
[curvatureRate, prevCurvature, angles] = rigidificator.computeBendRates(mesh3Da, mesh3Da.p, mesh3Da.prevCurvature,h);
mesh3Da.prevCurvature = prevCurvature;
be.integrate(mesh3Da, h, Jc, phi, cInfo, settings, cache, [], [], 1, [], []);
cache.F = mexComputeFtri3D(mesh3Da.getPositionFormatted,mesh3Da.t,mesh3Da.triNormals,mesh3Da.dphidx(:));

computeRegionOfWaking(mesh3Da, h, cache, be, settings, rigidificator, V1, V2, impulseVertIDs, impulse, zeros( 0, mesh3D.N*3), [], prevCurvature);
cache.ApproximatedDeltaV = zeros(size(cache.ApproximatedDeltaV));

function computeRegionOfWaking(mesh, h, cache, be, settings, rigidificator, V1, V2, impulseVertIDs, impulse, Jc, phi, prevCurvature)
    cache.clearWarmStartInfo();
    quickSolve3D( cache, be, mesh, h, Jc, [], settings, [], 1, contactInfo3D.empty); 
...
%         makeMeshRigid(mesh);
        rigidificator.ElastificationThreshold = 999999999999;
        rigidificator.RigidificationThreshold = 10 ^ (-i);
        rigidificator.setBendingThresholdsFromPlanar();
        rigidificator.Fprev = cache.oldF;
        mesh.prevCurvature = prevCurvature;
