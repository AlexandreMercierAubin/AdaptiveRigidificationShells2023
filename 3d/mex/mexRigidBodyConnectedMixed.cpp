 ...
 * To compile with debug: mex -g -R2018a mexRigidBodyConnectedMixed.cpp
 *========================================================*/
...
void visitComponent(const size_t rootIndex, mxLogical* isVertBoundary, mxLogical* isTriElastic,
                    ...
                    int &numRigid, std::queue<size_t> &toVisit,
       ...
       if(rigidIDbyTri[rootIndex] == 0){
        ...
        while(!toVisit.empty()){ 
            ...
            int localVertexSize = 4;
            bool isTri = (triVertIDint32[tId + 3 * numTets] -1) == -1;
            if (isTri){localVertexSize=3;}

            for (size_t v = 0; v < localVertexSize; ++v) {
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    if (numTets != mxGetM(prhs[3]) || 4 != vSize) {
        mexErrMsgIdAndTxt("ARP:rigidConnectedComponents:size", "triVertIDint32: Expecting a matrix element by 4 (4th must be 0 if tri)");
    }
    ...
    for ( size_t i = 0; i < numTets; ++i ) {
        ...
        int localVertexSize = 4;
        bool isTri = (triVertIDint32[i + 3 * numTets] -1) == -1;
        if (isTri){localVertexSize=3;}

        for (size_t v = 0; v < localVertexSize; ++v) { //init vert as elastic 
        ...
        if (eligible) { 
            ...
            for (size_t v = 0; v < localVertexSize; ++v) { //set eligible vertices as rigid
                int vertex = triVertIDint32[i + v * numTets] - 1; // index starts at 0 instead of 1
                isVertElastic[vertex] = false;
            }
        }
...
//         Forces elastification of every dofs of an elements tagged for rigidification. Perhaps overkill 
    ...
    for ( size_t i = 0; i < numStableTris; ++i ) {
        ...
        visitComponent(t, isVertBoundary, isTriElastic, isVertElastic,
                        ...
                        pinnedVerts, numRigid, toVisit, isComponentPinned, isStableTri, isAnimated, isAnimatedComponent);
    }
