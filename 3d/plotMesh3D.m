...
pr = reshape(mesh3D.p, 3, mesh3D.N)' + mesh3D.renderOffset;
plotFacesOnly = true;
edgeCol = [2 1 34] / 255;
...
if isa(mesh3D, 'AdaptiveMesh3D')
    ...
    elseif ~isempty(settings.DrawVertexNumbers)
        faceAlpha = 0.8;
        verts = settings.DrawVertexNumbers;
        for i = 1 : numel(verts)
            c = mesh3D.p([verts(i)*3-2,verts(i)*3-1,verts(i)*3]) + mesh3D.renderOffset;
            r = 0.05;
            ...
            hold on;
            if numel(mesh3D.DOFsSpheres) < numel(verts)
                plotHandle = surf(xdata,ydata,zdata,cdata,'EdgeColor','none','FaceColor',[0 0 0]);
                mesh3D.DOFsSpheres = [mesh3D.DOFsSpheres,plotHandle];
            else
                mesh3D.DOFsSpheres(i).XData = xdata;
    ...
    elseif settings.DrawRigidDOFs
        faceAlpha = 0.5;
        rigidDofs = [mesh3D.RigidBodies.VertexIDs];
        
        for i = 1 : mesh3D.N
            c = [mesh3D.p(3*i-2),mesh3D.p(3*i-1),mesh3D.p(3*i)] + mesh3D.renderOffset;
            r = 0.05;
...
if ~isempty(mesh3D.SurfaceWeights)
    V = reshape(mesh3D.p',3,[])' + mesh3D.renderOffset;
...
if settings.DrawVelocities
    for i = 1:mesh3D.N
        xdata = [mesh3D.p(i*3-2) + mesh3D.renderOffset(1);mesh3D.p(i*3-2) + mesh3D.renderOffset(1)+mesh3D.v(i*3-2)];
        ydata = [mesh3D.p(i*3-1)+ + mesh3D.renderOffset(2);mesh3D.p(i*3-1)+ + mesh3D.renderOffset(2)+mesh3D.v(i*3-1)];
        zdata = [mesh3D.p(i*3) + mesh3D.renderOffset(3);mesh3D.p(i*3) + mesh3D.renderOffset(3)+mesh3D.v(i*3)];
    ...
    for j = 1:numel(cInfos)
        ...
        if settings.DrawLambdas
            ...
            if numel(mesh3D.lineHandles) < j
                mesh3D.lineHandles = [mesh3D.lineHandles;line(xdata,ydata,zdata, 'Color',[100/255,100/255,255/255],"LineWidth",settings.DrawLambdasThickness)];
            else
