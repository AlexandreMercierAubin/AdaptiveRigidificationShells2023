classdef EDot3DMexRigidificator < Rigidificator3D
    ...
    methods
        ...
        function checkForElastification( obj, mesh3D, cache, frame, h, settings )
            ...
            rigidTrisHist(mesh3D.AlwaysRigidElements,:) = true;
            %End rigidification
            %============================================
            %Elastification Edot ====================
            ...
            trisToElastify(mesh3D.AlwaysRigidElements) = false;
            % TODO: IMPROVEMENT... cant currently rigidify triangles with
            % pinned vertices because we can't handle the constraint on the
            % rigid body.  This might not be too hard to add with the PGS
            % solver, or by allowing elements with two pinned vertices to
            % become rigid but pinned.
            %End elastification
            %============================================
            ...
            if frame >= obj.FrameCount || settings.FirstFrameRigidification
                if obj.PreventPinnedRigidification
                    trisToElastify(mesh3D.pinnedTets) = true;
                end
            ...
            for i = 1:numRigid
                ...
                mesh3D.RigidBodies(i).VertexIDs = find(rigidIDbyVert == i)';
                ...
                inds3= mesh3D.RigidBodies(i).VertexIDs * 3;
