    ...
    while ishghandle(mainFig) && notDone
        ...
        for k = 1:comparisons
            ...
            if ~isempty(settings.FocusOnMeshNode)
                ...
                target = [xPos,yPos,zPos]; %TODO add ;
                if isnan(target(1))
                    theMeshExploded = 1
                end
            ...
            if ~isa(integrator,'FullNewton3D') && ~isa(integrator, 'FullNewtonLinearized3D')
                for j = 1:numel(contactFinders)
            ...
            [mesh3D.B, cache.F]=addShellNormalDeformation(mesh3D, mesh3D.p,mesh3D.triNormals, settings);
            mesh3D.p = SVDStrainLimiting(mesh3D, mesh3D.p,cache, settings);

            

            [psi, cache.bendingForces,cache.shellBendingH, cache.shellBendingDampingH, cache.dihedralAngles] = computeBendingGradHess(mesh3D, mesh3D.p, mesh3D.triNormals, cache, settings);
            
            if settings.useGrinspunPlanarEnergy
                isShell = mesh3D.elementType == mesh3D.elementTypeEnum.Shell;
                [ ii, jj, vals, dpsidX , valsD, Wa, Wl] = mexGrinspunPlanar(mesh3D.p,mesh3D.t,mesh3D.elkl,mesh3D.elka,mesh3D.area,isShell, mesh3D.elAlpha1, mesh3D.grinspunEnergyEdgeRestLength, sum(isShell));
                cache.grinspunPlanarH = sparse(ii,jj,-vals,mesh3D.N*3,mesh3D.N*3);
                cache.grinspunPlanarHd = sparse(ii,jj,-valsD,mesh3D.N*3,mesh3D.N*3);
                ...
                cache.K = energyModel.stiffness + cache.shellBendingH;
                cache.Kd = energyModel.stiffnessDamping + cache.shellBendingDampingH;
            end
            ...
            if isa( mesh3D, 'AdaptiveMesh3D' )
                ...
                quickSolve3D( cache, integrator, mesh3D, h, Jc, phi, settings, animationScripters, frame, cInfos); 
            ...
            elseif settings.plotCurvatureComparison
                plotID  = k + 1;
  
                if rigidificator.bendType == 2
                    titleText = sprintf('diheral rate %d elements',size(mesh3D.t,1));
                else
                    titleText = sprintf('curvature rate %d elements',size(mesh3D.t,1));
                end
                histogram( axesList(plotID), real(log10( cache.bendingRate )), [-inf -7:.1:1 inf], 'Normalization', 'probability' );
                title(axesList(plotID),titleText);
                ylim( axesList(plotID), [0,0.1] );
            elseif(settings.PlotEigs)
                eigsContact = eigs(cache.JcAinvJcT);
                plot( axesList(2), eigsContact);
                title(axesList(2),'eigs JcAinvJcT');
                xlabel(axesList(2),'eigen value number');
                xlabel(axesList(2),'eigen value');

                eigsA = eigs(cache.A);
                plot( axesList(3), eigsA);
                title(axesList(3),'eigs A');
                xlabel(axesList(3),'eigen value number');
                xlabel(axesList(3),'eigen value');
...
%                 [edgeCurv,~] = rigidificator.computeBendRates(mesh3D,mesh3D.p,false, cache.prevPrevDihedralAngle);
                histogram( axesList(2), real(log10( cache.bendingRate )), [-inf -7:.1:1 inf], 'Normalization', 'probability' );
...
%                 xline( axesList(3), log10(rigidificator.RigidificationThreshold) );
            ...
            if ~settings.quicksolveSimulation
                integrator.integrate(mesh3D, h, Jc, phi, cInfos, settings, cache, td{k}, animationScripters, frame, energyModel, contactFinders);
            end
            elapsed = elapsed + h;
            ...
            %taking a snapshot of the scene
            if(cacheNextFrame)
                filename = [convertStringsToChars(settings.SceneName), 'f', convertStringsToChars(frame),'k', convertStringsToChars(k)];
                cachePath = ['3d/data/cached/' , filename, 'mesh', '.mat'];
                save(cachePath, 'mesh3D');
                %the rest should already be cached by that point (Ainv
                %preconditioner, etc.) You can use this feature by simple
                %changing the filename when calling fetchMesh
            end
    ...
    function onKeyPressed(~, event)
        if strcmp( event.Key, 'c' )
            disp( "Getting ready to cache next the frame");
            cacheNextFrame = true;
