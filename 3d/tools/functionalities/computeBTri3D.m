function [Dm, DmInv2D, B, dphidx] = computeBTri3D(V,T,N)
   ...
   for i = 1:numt
        ...
        Dm(:,:,i) = localT'*localT;
        DmInv2D(:,:,i) = inv(Dm(:,:,i));
        DmInvLocalT = DmInv2D(:,:,i)*localT';
        
        if nargout > 2
            %local B block for individual elements
            D = [-sum(DmInvLocalT,1);DmInvLocalT] ;
            dphidx(:,:,i) = [D;N(i,:)]; %adds a row of zeros so it can be stacked with tet D 
