...
%                         writeOBJ([settings.OBJDir 'frame_surface_' num2str(k,'%03.f') '_' num2str(frame/(settings.PlotSkip+1),'%03.f'), '.obj'], nV, mesh3D.SurfaceFaces);
   ...
   if isa(mesh3D, "AdaptiveMesh3D")
        inds = find(mesh3D.isTetElastic);
    else
        inds = 1:size(mesh3D.t);
   end
   inds = unique(inds);
   ...
   if settings.ExportNormalsOBJs
       N = normals(V,Fel, 'Stable', false, 'UseSVD', true);
       for i = 1 : size(N,1)
           normN = norm(N(i,:));
           N(i,:) = N(i,:)/normN;
       end
       writeOBJ([settings.OBJDir 'frame_' num2str(k,'%03.f') '_' num2str(frame/(settings.PlotSkip+1),'%03.f'), '.obj'], V, Fel, [], [],N);
   else
       writeOBJ([settings.OBJDir 'frame_' num2str(k,'%03.f') '_' num2str(frame/(settings.PlotSkip+1),'%03.f'), '.obj'], V, Fel);
   end
