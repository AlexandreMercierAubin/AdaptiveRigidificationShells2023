    ...
    if toRecompute
        ...
        for k = 1:comparisons
            ...
            if ( isa(mesh3D, 'AdaptiveMesh3D') )   
                disp("precomputing preconditioner");
                precomputeAInverseDiagonalBlocks3D( cache, mesh3D, h, energyModel,settings);
                disp("precomputed block diagonals");
...
%                         struct('type','nofill','michol','on') ) ) ); % Go
                ...
                cache.Perm = Perm;
                cache.Lpre = Lpre;
                disp("precomputing preconditioner: done");
            end
        end
        save(cachedAinvString,'caches');
    end
