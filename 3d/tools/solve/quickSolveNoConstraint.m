...
%             contactVertices = [cache.cInfo.vertexID];
...
% %                 contactRows = 1:size(Jc,1); %with friction
    ...
    warmStartDv = zeros(size(rhs)); %TODO: makes this use the first contact dv as warmstarts like in 3D
    if integrator.separateQuicksolveGravity
        warmStartDv = warmStartDv - cache.gravityDv;
    end

    cache.ApproximatedDeltaV(ii) = solvePCG( settings.PCGiterations, Afun, rhs, warmStartDv, cache ) + cache.gravityDv;
